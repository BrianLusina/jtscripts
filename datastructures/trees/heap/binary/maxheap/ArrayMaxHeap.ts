...
/**
 * ArrayMaxHeap is an abstract data structure which implements a MaxHeap using an array as the 
 ...
 */
export default class ArrayMaxHeap<T> extends ArrayHeap<T> {
    ...
    insert(data: T): void {
        ...
        // if the new node is not in the root position and it's greater than it's parent node
    ...
    delete(): T {
        ...
        // below is the trickle down algorithm. This is run as long as the trickle node has a child that is greater than it
        while(this.hasGreaterChild(trickleNodeIndex)){
            // retrieve the index of the larger child for this node
            const largerChildIndex = this.getLargerChildIndex(trickleNodeIndex)

            // swap the trickle node with it's larger child

            // temp is used here to ensure that we don't lose the value of the node at the largerChildIndex
            const temp = this.items[largerChildIndex]
            this.items[largerChildIndex] = this.items[trickleNodeIndex]
    ...
    /**
     * Checks if the node at the given index has a child that is greater than it.
     * Returns true if that is the case
     * @param {number} index Index of node to check for greater child
     * @returns {boolean} true if the node has a greater child, false otherwise 
     */
    private hasGreaterChild(index: number): boolean {
        ...
        return this.items[leftChildIdx] > this.items[index] || this.items[rightChildIdx] > this.items[index]
    }

    /**
     * Retrieves the index of the node's larger child and returns it.
     * @param index Index of the node in the heap
     * @returns {number} index of the larger child
     */
    private getLargerChildIndex(index: number): number {
        ...
        // at this point, the node has both left & right children, check which is greater
