 ...
 * @param {number[]} nums array of integers with duplicate peaks
 * @returns {number} index of a peak element(i.e. element that is greater than its adjacent neighbours)
 */
export function findPeakElement(nums: number[]): number {
  ...
  while (left <= right) {
    ...
    const potentialPeak = nums[mid];

    if (mid == nums.length - 1 || potentialPeak > nums[mid + 1]) {
      peakIndex = mid;
      right = mid - 1;
    } else {
      left = mid + 1;
